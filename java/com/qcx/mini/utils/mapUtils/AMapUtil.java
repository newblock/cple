...
public class AMapUtil {
    ...
    public static void setPointToCenter(final AMap aMap, final View viewById, final Context context) {
        viewById.post(new Runnable() {
            @Override
            public void run() {
                int width = viewById.getWidth();
                int height = viewById.getHeight() + UiUtils.getStatusBarHeight(context);
    ...
    public static int getFriendlyLength2Int(int lenMeter) {
        if (lenMeter > 10000) // 10 km
        {
            int dis = lenMeter / 1000;
            return dis;
    ...
    public static String getFriendlyTime(long second) {
        if (second > 3600) {
    ...
    public static int getFriendlyTime2(int second) {
        if (second > 3600) {
            int hour = second / 3600;
            int miniate = (second % 3600) / 60;


            return miniate + (hour * 60);
        }
        if (second >= 60) {
            return second / 60;
        }
