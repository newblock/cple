...
public class FastBlurUtility {
     ...
     * @return 模糊化的背景图片
     */
    public static Bitmap getBlurBackgroundDrawer(Activity activity,int startX,int startY,int width,int height) {
        Bitmap bmp = takeScreenShot(activity,startX,startY,width,height);
     ...
     * @param startX 图片开始的X的值
     * @param startY 图片开始的Y的值
     * @param width 图片宽度
     * @param height 图片高度
     * @return 截取的图片
     */
    private static Bitmap takeScreenShot(Activity activity,int startX,int startY,int width,int height) {
        View view = activity.getWindow().getDecorView();
        view.setDrawingCacheEnabled(true);
        view.buildDrawingCache();
        Bitmap b1 = view.getDrawingCache();

        Bitmap bmp = Bitmap.createBitmap(b1, startX, startY, width, height);
