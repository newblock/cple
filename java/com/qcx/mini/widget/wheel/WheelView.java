...
public class WheelView extends View {
    ...
    int startX=0;//中间选中文字末尾提示文字的位置

    private boolean isShowReminderText=false;
...
//        textSize = getResources().getDimensionPixelSize(R.dimen.pickerview_textsize);//默认大小
        textSize=(int)(17* UiUtils.getPixelH()/375.0);
    ...
    private void initPaints() {
        ...
        reminderText = new Paint();
        reminderText.setColor(0xFFFF7779);
        reminderText.setAntiAlias(true);
//        reminderText.setTextScaleX(0.8F);
        reminderText.setTypeface(typeface);
        reminderText.setTextSize(7*UiUtils.getPixelV()/375);
...
//                canvas.scale(1.0F, (float) Math.sin(radian));
        ...
        String rText=getReminderText();
        if(isShowReminderText&&!TextUtils.isEmpty(rText)){
            canvas.drawText(getReminderText(), startX, secondLineY-(secondLineY-firstLineY)/4, reminderText);
        }
    }
    ...
    private String getReminderText(){
        Object o=adapter.getItem(getCurrentItem());
       if(o!=null&&o instanceof IPickerViewReminderData ){
           return ((IPickerViewReminderData)o).getReminderText();
       }else {
           return " ";
       }
    }

    public void setShowReminderText(boolean isShowReminderText){
        this.isShowReminderText=isShowReminderText;
    }
        ...
        switch (mGravity) {
            case Gravity.CENTER://显示内容居中
                if (isOptions || label == null || label.equals("") || !isCenterLabel) {
                    ...
                    if(rect.width()>10){
                        startX = (int) ((measuredWidth + rect.width()) * 0.5+5);
                    }
                    if(startX==0)startX=(int)(measuredWidth*0.5+100);
